...
if [ $? -eq 0 ]; then
  echo "[OK] Security Group pettools-dev foi criado"
  
  # Validar inbound rule para o security group 'pettools-dev'
  ...
  # Validar outbound rule para o security group 'pettools-dev'
  ...
  echo ">[ERRO] Não achei o security group pettools-dev. Ele foi criado?"
fi
